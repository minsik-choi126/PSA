#!/usr/bin/env bash
# One-shot: PSA (Phase A + Phase B) → TSV → printed summary table.
# Usage:
...
PSA_OUT="${THIS_DIR}/merged_psa"
TSV_OUT="${THIS_DIR}/merged_tsv"
LOG_DIR="${THIS_DIR}/logs"
mkdir -p "${LOG_DIR}"

echo "════════════════════════════════════════════════════════════════"
echo "  Runtime benchmark: PSA → TSV    $(date '+%F %T')"
echo "  DP for PSA.PhaseA = ${DP}    Merge for both = 1 GPU (cuda:0)"
echo "════════════════════════════════════════════════════════════════"

# ─── PSA.PhaseA ─────────────────────────────────────────────────────
rm -f "${W_OUT}"; rm -rf "${PSA_OUT}"
echo ""
echo "──── PSA / Phase A: W_col extraction (DP=${DP}) ────"
TA0=$(date +%s.%N)
...
# ─── PSA.PhaseB ─────────────────────────────────────────────────────
echo ""
echo "──── PSA / Phase B: PSA merge (1 GPU) ────"
TB0=$(date +%s.%N)
python "${THIS_DIR}/merge_psa.py" \
    --base_model "${BASE_MODEL}" \
    --expert_coding "${EXPERT_CODING}" \
    --expert_tool "${EXPERT_TOOL}" \
    --expert_memory "${EXPERT_MEMORY}" \
...
# ─── Summary ─────────────────────────────────────────────────────────
T_PSA=$(awk "BEGIN{printf \"%.2f\", ${T_W} + ${T_M}}")
fmt() {
...
printf "  %-32s  %12s  %s\n" "PSA / Phase A (W extract DP=${DP})" "${T_W}" "$(fmt ${T_W})"
printf "  %-32s  %12s  %s\n" "PSA / Phase B (merge 1 GPU)"        "${T_M}" "$(fmt ${T_M})"
printf "  %-32s  %12s  %s\n" "PSA / Total"                        "${T_PSA}" "$(fmt ${T_PSA})"
printf "  %-32s  %12s  %s\n" "TSV   / Merge (1 GPU)"               "${T_T}" "$(fmt ${T_T})"
echo ""
echo "  outputs: ${PSA_OUT} , ${TSV_OUT}"
echo "  logs:    ${LOG_DIR}/"
echo "════════════════════════════════════════════════════════════════"
