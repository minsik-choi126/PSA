"""Policy-Shift-Guided Spectral Alignment merge for 3 RL experts on a single GPU.
...
def psa_col_truncate(tau: torch.Tensor, W_col: torch.Tensor | None,
...
def resvd_via_qr(cache: dict):
    """Original-space orthonormal U,S,V from psa_col_truncate cache via QR+small SVD.
...
def merge_one_layer(taus, W_col_per_expert, energy, device):
    """PSA path for a single 2D layer."""
    N = len(taus)
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="PSA merge (1 GPU)")
