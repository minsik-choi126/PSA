...
|---|---|---|
| **PSA** (Policy-Shift-Guided Spectral Alignment) | **W extraction** (Phase A) + **Merge** (Phase B) | A = DP=N (configurable),  B = 1 GPU |
...
├── merge_psa.py               # PSA merge (single GPU, optimized)
├── merge_tsv.py                # TSV merge (single GPU, baseline)
├── run_psa.sh                 # PSA wrapper: Phase A (DP=N) + Phase B + timing
...
```bash
bash run_psa.sh                  # default DP=2  (uses cuda:0 + cuda:1 for Phase A)
bash run_psa.sh --dp 4           # 4-GPU machine: 4-way prompt sharding for Phase A
bash run_psa.sh --dp 1           # single-GPU run (slowest, but apples-to-apples vs TSV)
```

Outputs (in this folder):

- `W_col_neg_top10.npz`      — extracted W matrix
- `merged_psa/`             — final merged HF checkpoint (safetensors)
