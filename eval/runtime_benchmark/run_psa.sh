...
# Phase B: PSA merge on 1 GPU (cuda:0, fair vs baselines)
#
# Models: pass HF id or local path via env vars. Defaults match canonical
# 7B Qwen2.5 RL experts.
#
# Usage:
#   bash run_psa.sh                          # DP=2
#   bash run_psa.sh --dp 4                   # DP=4
#   BASE_MODEL=Qwen/Qwen2.5-7B-Instruct bash run_psa.sh
set -euo pipefail
...
MERGE_OUT="${THIS_DIR}/merged_psa"
LOG_DIR="${THIS_DIR}/logs"
mkdir -p "${LOG_DIR}"

echo "════════════════════════════════════════════════════════════════"
echo "  PSA runtime benchmark   $(date '+%F %T')"
...
echo ""
echo "════════════════════════════════════════════════════════════════"
echo "  PSA runtime SUMMARY   (DP=${DP})"
echo "════════════════════════════════════════════════════════════════"
